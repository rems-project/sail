...
let rec value_cmp f lhs rhs =
  ...
  | V_vector l_v ->
    (match rhs with
     | V_vector r_v -> list_eqb (value_cmp f) l_v r_v
     | V_unknown -> f lhs
     | _ -> false)
  | V_list l_xs ->
    (match rhs with
     | V_list r_ys -> list_eqb (value_cmp f) l_xs r_ys
     | V_unknown -> f lhs
     | _ -> false)
  | V_int l ->
    (match rhs with
  ...
  | V_tuple l_v ->
    (match rhs with
     | V_tuple r_v -> list_eqb (value_cmp f) l_v r_v
     | V_unknown -> f lhs
     | _ -> false)
  ...
  | V_ctor (l_id, l_v) ->
    (match rhs with
     | V_ctor (r_id, r_v) ->
       (&&) (id_eqb l_id r_id) (list_eqb (value_cmp f) l_v r_v)
     | V_unknown -> f lhs
     | _ -> false)
  | V_record l_fields ->
    (match rhs with
     | V_record r_fields ->
       list_eqb (fun pat pat0 ->
         let (l_id, l_v) = pat in
         let (r_id, r_v) = pat0 in
         (&&) (id_eqb l_id r_id) (value_cmp f l_v r_v)) l_fields r_fields
     | V_unknown -> f lhs
     | _ -> false)
  | V_unknown -> (match rhs with
                  | V_unknown -> f lhs
                  | _ -> false)
...
let value_eqb lhs rhs =
  value_cmp is_unknown lhs rhs
