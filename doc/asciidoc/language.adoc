...
=== Vectors
:vectorex: sail_doc/bitvector_and_generic.json
...
sail::same_order[from=vectorex,part=body,dedent]
...
There is a slight variation of the `union` keyword, which allows
constructing a union with just a single constructor:
...
[source,sail]
----
union register_index = {
  Mk_index : bits(5)
}
----

The main difference from is that for certain Sail targets (including
C, SMT, and SystemVerilog) `register_index` will be compiled as if it
were `bits(5)` directly.

The purpose of the `newtype` construct is to create an alias for an
existing type which is distinct from the perspective of the type
system -- We cannot accidentally pass any `bits(5)` where a
`register_index` index is required, as register indices must have been
explicitly constructed using `Mk_index`. This means that using
`newtypes` can increase type-safety and therefore prevent errors,
unlike regular type synonyms which we discuss below.
...
ISA width (called XLEN in RISC-V).

In Sail, we could write something like:
[source,sail]
----
$ifdef RV32
type xlen = 32
...
----
However, this has the problem that if we write a function like:
[source,sail]
----
function rv32_only() = {
    X1 = 0xFFFF_FFFF
...
----
it will typecheck and build fine only when `RV32` is defined, but will
fail otherwise when `xlen = 64`.

What we really want is to write a specification that is truely
_parametric_ over the choice of `xlen`. In Sail we can do this by
leaving the `xlen` type _abstract_, which means we do not provide a
concrete value for it during type-checking. This would look like:

sail::XLEN[from=abstract-ty,type=span]

If we then try to write the above `rv32_only` function, we will get an error
[source]
----
include::sail_doc/abstract_error.error[]
----
as `xlen` is not guaranteed to be 32.

Not all Sail targets support abstract types, so for some targets the
abstract types must be instantiated to concrete values before
generating definitions. We do this using the `--instantiate` option,
...
=== Registers
:reg: sail_doc/register.json

We have discussed mutable variables defined with `var`, and assignment
to those variables in the previous sections. The global state of an
ISA is defined in Sail by a set of _registers_, which essentially act
as global mutable variables. For example, we could define and use a register
as follows:

sail::EXAMPLE[from=reg,type=span]

We can provide a default initial value to a register, or leave it
undefined. In the above example the register definition is equivalent
to writing

[source,sail]
----
register R : bits(64) = undefined
----

For readability reasons, local variables are not permitted to shadow
global register definitions, as the following example demonstrates.

[source,sail]
----
include::../examples/register_shadow.sail[]
----

[source]
----
include::sail_doc/register_shadow.error[]
----
...
`try`--`catch` blocks. The throw keyword takes a value of type
`exception` as an argument, which can be any user defined type with
that name. There is no built-in exception type, so to use exceptions
one must be set up on a per-project basis. The exception type must be
a union (except it cannot be declared via `newtype`), and is usually a
scattered union to allow exceptions to be declared throughout the
specification as they would be in OCaml, for example:
...
:log2: sail_doc/log2.json

Many theorem provers work best when all functions are completely well
defined, and in particular that any use of loops or recursion
terminates.  Termination measures help by giving an upper bound on the
number of iterations of a loop, or the number of recursive calls,
which can be calculated using the variables currently in scope and
decreases each time.

sail::log2_while[from=log2]

Loops using `foreach` are naturally terminating and do not require a
measure.

Termination measures can also be given separately from the code.  This
is useful for adding measures to a specification without altering it,
or for compatibility with older versions of Sail.

[source,sail]
----
termination_measure log2_while while (n)
----

Recursive functions can be given termination measures in a
similar way.

[source,sail]
----
include::sail:log2_recursive[from=log2]

termination_measure log2_recursive(n) = n
----

The Lean output of Sail uses the termination measures along with a
default proof tactic to prove termination.  Currently, the Rocq output
uses the measures as a limit asserted within the function, rather than
