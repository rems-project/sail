theory Sail2_concurrency_interface_v2_lemmas
  imports
...
| Read_mem: "((Mem_read req k), E_mem_read req v, k v) \<in> T"
| Write_mem: "((Mem_write req data tags k), E_mem_write req data tags r, k r) \<in> T"
| Write_announce: "((Mem_write_address_announce req k), E_mem_write_address_announce req, k) \<in> T"
...
| Fault: "((Take_exception exn k), E_take_exception exn, k) \<in> T"
| Eret: "((Return_exception k), E_return_exception, k) \<in> T"
...
lemma emitEvent_cases:
  ...
  | (Read_mem) req k v where "m = Mem_read req k" and "e = E_mem_read req v" and "m' = k v"
  | (Write_mem) req data tags k r where "m = Mem_write req data tags k" and "e = E_mem_write req data tags r" and "m' = k r"
  | (Write_announce) req k where "m = Mem_write_address_announce req k" and "e = E_mem_write_address_announce req" and "m' = k"
  ...
  | (Take_exception) exn k where "m = Take_exception exn k" and "e = E_take_exception exn" and "m' = k"
  | (Return_exception) k where "m = Return_exception k" and "e = E_return_exception" and "m' = k"
...
lemma Traces_cases:
  ...
  | (Read_mem) req k t' v where "m = Mem_read req k" and "t = E_mem_read req v # t'" and "(k v, t', m') \<in> Traces"
  | (Write_mem) req data tags k v t' where "m = Mem_write req data tags k" and "t = E_mem_write req data tags v # t'" and "(k v, t', m') \<in> Traces"
  | (Write_announce) req k t' where "m = Mem_write_address_announce req k" and "t = E_mem_write_address_announce req # t'" and "(k, t', m') \<in> Traces"
  ...
  | (Take_exception) exn k t' where "m = Take_exception exn k" and "t = E_take_exception exn # t'" and "(k, t', m') \<in> Traces"
  | (Return_exception) k t' where "m = Return_exception k" and "t = E_return_exception # t'" and "(k, t', m') \<in> Traces"
...
lemma Traces_iffs:
  ...
  "\<And>req k t m'. (Mem_read req k, t, m') \<in> Traces \<longleftrightarrow> (t = [] \<and> m' = Mem_read req k \<or> (\<exists>a t'. t = E_mem_read req a # t' \<and> (k a, t', m') \<in> Traces))"
  "\<And>req data tags k t m'. (Mem_write req data tags k, t, m') \<in> Traces \<longleftrightarrow> (t = [] \<and> m' = Mem_write req data tags k \<or> (\<exists>a t'. t = E_mem_write req data tags a # t' \<and> (k a, t', m') \<in> Traces))"
  "\<And>req k t m'. (Mem_write_address_announce req k, t, m') \<in> Traces \<longleftrightarrow> (t = [] \<and> m' = Mem_write_address_announce req k \<or> (\<exists>t'. t = E_mem_write_address_announce req # t' \<and> (k, t', m') \<in> Traces))"
  ...
  "\<And>exn k t m'. (Take_exception exn k, t, m') \<in> Traces \<longleftrightarrow> (t = [] \<and> m' = Take_exception exn k \<or> (\<exists>t'. t = E_take_exception exn # t' \<and> (k, t', m') \<in> Traces))"
  "\<And>k t m'. (Return_exception k, t, m') \<in> Traces \<longleftrightarrow> (t = [] \<and> m' = Return_exception k \<or> (\<exists>t'. t = E_return_exception # t' \<and> (k, t', m') \<in> Traces))"
...
lemma Mem_write_TracesE:
  assumes "(Mem_write req data tags k, t, m') \<in> Traces"
  obtains (Nil) "t = []" and "m' = Mem_write req data tags k"
  | (Cons) t' r where "t = E_mem_write req data tags r # t'" and "(k r, t', m') \<in> Traces"
  using assms
  by (auto elim: Traces_cases)
