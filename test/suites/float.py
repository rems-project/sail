...
class FloatTests(SailTest):
    def prepare(self):
        shutil.copy(
            os.path.join(_SUITE_DIR, "data.sail"),
            os.path.join(self.work_dir, "data.sail"),
        )
        shutil.copy(
            os.path.join(_SUITE_DIR, "tuple_equality.sail"),
            os.path.join(self.work_dir, "tuple_equality.sail"),
        )
    ...
    def _test(self, test):
        ...
        step(
            f"diff {test.basename}.err_result {test.directory}/no_error && rm {test.basename}.err_result"
        )
