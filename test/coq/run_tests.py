...
class CoqTests(SailTest):
    def run(self):
        ...
        self.run_tests('typecheck tests on stdpp', os.listdir('../typecheck/pass'),
                       self._make_test('../typecheck/pass', 'stdpp'),
                       expected_failures=_common_xfails, skip_set=skip_tests)

        banner('Testing Coq backend on Coq specific tests with stdpp')
        self.run_tests('Coq specific tests on stdpp', os.listdir('pass'),
        ...
        try:
            ...
            if p.returncode == 0:
                ...
                self.run_tests('typecheck tests on bbv', os.listdir('../typecheck/pass'),
                               self._make_test('../typecheck/pass', 'bbv'),
                               expected_failures=bbv_xfails, skip_set=skip_tests)

                banner('Testing Coq backend on Coq specific tests with bbv')
                self.run_tests('Coq specific tests on bbv', os.listdir('pass'),
    ...
    def _make_test(self, dir, lib):
        def fn(filename, basename):
            ...
            step('\'{}\' --coq --coq-lib-style {} --dcoq-undef-axioms --strict-bitvector --coq-output-dir _build_{} -o out {}/{}'.format(
