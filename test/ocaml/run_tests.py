...
class OcamlTests(SailTest):
    def run(self):
        ...
        if 'ocaml' in targets:
            ...
            self.run_tests('Ocaml testing', os.listdir('.'), self._make_test(''),
                           chunks_fn=directory_chunks)

        if 'ocaml_trace' in targets:
            banner('Ocaml trace testing with options: "--ocaml-trace"')
            self.run_tests('Ocaml trace testing', os.listdir('.'), self._make_test('--ocaml-trace'),
                           chunks_fn=directory_chunks)

    def _make_test(self, opts):
        def fn(dir, basename):
            step(f'{self.sail} --strict-bitvector --no-warn -o out --ocaml {opts} ../prelude.sail *.sail', cwd=dir)
