...
class FormatTests(SailTest):
    def run(self):
        ...
        self.run_tests('lw80_preserve', os.listdir('.'), self._make_test('lw80_preserve'))

    def _make_test(self, test_dir):
        def fn(filename, basename):
            step(f'cp {filename} {test_dir}/{filename}')
            step(f'\'{self.sail}\' --sail-config {test_dir}/config.json --fmt {test_dir}/{filename}')
            ...
            if status != 0:
                if args.update_expected:
                    ...
                    step(f'\'{self.sail}\' --sail-config {test_dir}/config.json --fmt {filename} --fmt-emit stdout > {test_dir}/{basename}.expect')
                else:
                    sys.exit(1)
