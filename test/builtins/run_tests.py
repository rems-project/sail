...
class BuiltinsTests(SailTest):
    def run(self):
        ...
        if 'c' in targets:
            ...
            self.run_tests('C, No optimisations', os.listdir('.'), self._make_c_test(''))

            banner('Testing builtins: C, Optimisations Sail options: -O')
            self.run_tests('C, Optimisations', os.listdir('.'), self._make_c_test('-O'))

            banner('Testing builtins: C, Constant folding Sail options: -Oconstant_fold')
            self.run_tests('C, Constant folding', os.listdir('.'), self._make_c_test('-Oconstant_fold'))
    ...
    def _make_c_test(self, sail_opts):
        def fn(filename, basename):
            step('\'{}\' -no_warn -c {} {} -o {}'.format(self.sail, sail_opts, filename, basename))
            step('gcc {}.c \'{}\'/lib/*.c -lgmp -I \'{}\'/lib -o {}'.format(
    ...
    def _test_ocaml(self, filename, basename):
        step('\'{}\' -no_warn -ocaml -ocaml_build_dir _sbuild_{} -o {} {}'.format(
    ...
    def _test_coq(self, filename, basename):
        step('\'{}\' --no-warn --coq --coq-lib-style stdpp --coq-record-update --undefined-gen -o {} {}'.format(
        ...
        step('coqtop -require-import {}_types -require-import {} -l test.v -batch | tee /dev/stderr | grep -q OK'.format(
            basename, basename))
        os.chdir('..')
    ...
    def _test_isla(self, filename, basename):
        ...
        step('\'{}\'/isla-sail/isla-sail {} \'{}\'/lib/vector_dec.sail \'{}\'/test/property/include/config.sail -o {}'.format(
            isla_dir, filename, self.sail_dir, isla_dir, basename))
        step('\'{}\'/target/release/isla-execute-function -A {}.ir -C \'{}\'/configs/plain.toml main'.format(
