...
libraries = ['values', 'operators', 'instr_kinds', 'prompt_monad', 'prompt',
             'operators_mwords', 'state_monad', 'state', 'string', 'undefined']
joiner = ' '
libpaths = joiner.join(['{}/src/gen_lib/sail2_{}.lem'.format('{}', lib) for lib in libraries])
...
class MonoTests(SailTest):
    ...
    def _test(self, filename, basename):
        ...
        step('\'{}\' --lem --lem-mwords --lem-lib Test_extra --lem-output-dir _build_{} -o out {}'.format(
            self.sail, filename, arguments))
        os.chdir('_build_{}'.format(filename))
        step('lem -ocaml -lib {}/src/lem_interp {} -outdir . ../test_extra.lem out_types.lem out.lem'.format(
